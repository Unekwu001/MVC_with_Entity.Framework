...
namespace Hotel.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            List<Models.HotelModel> AllHotels = _hotelRepository.GetHotels();

            //filter for most picks
            var mostpicks = AllHotels.Where(prop => prop.HotelPopularity == "Most Picks").ToList();
            ViewData["mostpicks"] = mostpicks;

            //filter for Houses with beautiful Backyards
            var backyards = AllHotels.Where(prop => prop.HotelDescription == "Houses with beautiful Backyards").ToList();
            ViewData["backyards"] = backyards;

            //filter for Hotels with large living rooms
            var livingRooms = AllHotels.Where(prop => prop.HotelDescription == "Hotels with large living rooms").ToList();
            ViewData["livingRooms"] = livingRooms;

            //filter for Apartments with Kitchen set
            var withKitchen = AllHotels.Where(prop => prop.HotelDescription == "Apartments with Kitchen set").ToList();
