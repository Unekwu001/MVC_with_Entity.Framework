...
namespace Hotel.Controllers
{
    public class DetailsController : Controller
    {
        ...
        {
            Models.HotelModel hotel = _hotelRepository.GetHotelById(id);
            ViewData["hotel"] = hotel;


            // Shuffle the hotels list using a random seed
            Random random = new Random();
            List<Models.HotelModel> shuffledHotels = _hotelRepository.GetHotels().OrderBy(x => random.Next()).ToList();

            // Select the first 4 hotels from the shuffled list
            List<Models.HotelModel> randomHotels = shuffledHotels.Take(4).ToList();
